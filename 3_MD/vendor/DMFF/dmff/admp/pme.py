...
def calc_e_ind(dr, thole1, thole2, dmp, pscales, dscales, kappa, lmax=2):
    ...
    # Match the AMOEBA reference implementation: use the smaller site thole
    # and apply the damping in u^3 rather than the MPID-style linear-u form.
    a = jnp.minimum(thole1, thole2)
    au3 = trim_val_infty(a * u * u * u)
    expau3 = jnp.piecewise(
        au3, [au3 < 50, au3 >= 50], [lambda x: jnp.exp(-x), lambda x: jnp.array(0)]
    )
    a2u6 = trim_val_infty(au3 * au3)
    a3u9 = trim_val_infty(a2u6 * au3)

    # AMOEBA Thole damping factors for energies and their radial derivatives.
    thole_c = 1.0 - expau3
    thole_d0 = 1.0 - expau3 * (1.0 + 1.5 * au3)
    thole_d1 = 1.0 - expau3
    thole_q0 = 1.0 - expau3 * (1.0 + au3 + a2u6)
    thole_q1 = 1.0 - expau3 * (1.0 + au3)
    dthole_c = 1.0 - expau3 * (1.0 + 1.5 * au3)
    dthole_d0 = 1.0 - expau3 * (1.0 + au3 + 1.5 * a2u6)
    dthole_d1 = 1.0 - expau3 * (1.0 + au3)
    dthole_q0 = 1.0 - expau3 * (1.0 + au3 + 0.25 * a2u6 + 0.75 * a3u9)
    dthole_q1 = 1.0 - expau3 * (1.0 + au3 + 0.75 * a2u6)
